...
vector<double> getmin(vector<double> a,int n)
{     vector<double> b;
	for(int i=0;i<n;i++)
		b.push_back(a[i]);
...
vector<double> getmax(vector<double> a,int n)
{
    vector<double> b;
	for(int i=0;i<n;i++)
		b.push_back(a[i]);
	for(int i=n;i<a.size();i++)
	{    b.push_back(a[i]);
		sort(b.begin(),b.end(),compare);   //Here n is either 2 or 3. sorting is done in such a way that it take O(n) time complexity i.e.sorting two element at a time=>O(2*n)
		                                   //O(2*log(2)*n)=O(n)
		b.pop_back();
	}
	return b;
...
{
vector<double> b3;
b3=getmax(A,3); // used to get max 3 elements in O(n) time
double m=0;
...
{
vector<double> b4;
b4=getmax(A,2); // used to get max 2 elements in O(logn) time
...
{
vector<double> b1;
b1=getmin(A,2);       //used to get min 2 elements in O(n) time
...
{
vector<double> b2;
b2=getmin(B,2);       //used to get min 2 elements in O(n) time
double m3=0;
